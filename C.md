...
**滑动窗口**是一种在 TCP 协议中用于 **流量控制** 的机制，用来限制发送方可以发送但尚未被确认的数据量。

📌 滑动窗口的本质是：

> 发送方根据接收方通告的接收窗口（`rwnd`）和自身维护的发送窗口来控制数据发送范围。
...
窗口的起始位置和终止位置会不断 **向前移动（滑动）**，所以称为“滑动窗口”。
...
2. 🟨 已发送但未确认的报文段  
   → `[base, nextseqnum)`，正在等待 ACK

3. 🟦 可发送但尚未发送的报文段  
   → `[nextseqnum, base + N)`，窗口允许的范围
...
- 🔄 **提升吞吐量**：可以连续发送多个包，无需每发一个就等待确认（与停止等待协议相比）；
- 🚦 **流量控制**：接收方通过 `rwnd` 控制窗口大小，防止自己被发送方“压垮”；
- 🔁 **实现可靠传输**：结合序列号、确认机制、重传机制等，保证数据可靠交付；
- 🔂 **与拥塞控制结合**：发送窗口还受到拥塞窗口 `cwnd` 的限制。

---

## ✅ 五、一句话总结

> **滑动窗口机制通过控制未确认数据的范围，使 TCP 能高效、可靠地进行流量控制和连续数据传输。**
